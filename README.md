# 🏠 Home Page

* [Useful Website Links](./links.md)
* [Common Bioinformatics Tools (Curated Spreadsheet)](https://docs.google.com/spreadsheets/d/19aNU64J1diFTT1soe4DDKa7pmG6O8MxQDxvi2kQzsTI/edit?usp=sharing)

---

📌 This [e-book](https://benson1231.gitbook.io/benson1231-docs) documents my ongoing journey in **bioinformatics engineering**, integrating concepts from **data science, software development, and biomedical research**. Its aim is to systematically curate this learning process and develop a collection of **searchable and reusable knowledge resources**.

For a complete overview, see the [📚 Table of Contents](./catalog.md).

---

## 📖 Topics Covered

* [Next-generation sequencing (NGS)](./01.NGS/README.md) data analysis, including DNA-seq, RNA-seq, single-cell RNA-seq, and microbiome workflows
* [Data science](./02.data-science/README.md) methodologies, covering data preprocessing, analysis, and visualization
* [Statistical](./03.statistics/README.md) foundations and applied statistical analysis for biomedical and computational research
* [DevOps](./04.devOps/README.md) practices, including containerization, CI/CD, workflow automation, and reproducible research environments
* [Backend](./05.back-end/README.md) development, focusing on API design, server-side logic, and service architecture
* [Frontend](./06.front-end/README.md) development, with modern web technologies and frameworks
* [Cloud and networking](./07.cloud&network/README.md) concepts, including cloud infrastructure, container orchestration, and networking fundamentals
* Core [computer science](./08.computer-science/README.md) topics, such as algorithms, data structures, and system-level concepts
* [Programming language syntax and usage](./09.lang-syntax/README.md), with practical examples across commonly used languages
* [Miscellaneous topics](./10.others/README.md) and supplementary notes

---

## 🚀 Featured Projects

* [snakemake-rnaseq pipeline](https://github.com/benson1231/snakemake-rnaseq): A modular and reproducible RNA-seq analysis pipeline supporting Docker, Conda, and CI/CD—designed for both research and teaching applications.

---

## 🔄 Updates & Contact

This e-book is actively maintained and will be updated on an ongoing basis. Comments, corrections, and scholarly or technical feedback are welcome.

🌐 Personal Website: [Visit My Website](https://benson1231.github.io/)  
📩 Contact: [benson106075@gmail.com](mailto:benson106075@gmail.com)  
🔗 LinkedIn: [Benson Lee](https://www.linkedin.com/in/chin-yu-lee-471109294)  
💻 GitHub: [benson1231](https://github.com/benson1231)  
🐳 Docker Hub: [benson1231](https://hub.docker.com/u/benson1231)  
